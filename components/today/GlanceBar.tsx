    ...
    <div className="glass-card p-3 mb-4">
      ...
      >
        <div className="flex items-center gap-2 flex-1">
          <Sun className="w-4 h-4" style={{ color: 'var(--text-muted)' }} strokeWidth={1.5} />
          <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{weatherSummary || 'Loading weather...'}</span>
        </div>
        {hasContent && (
          <div className="flex items-center gap-2">
            <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Tomorrow</span>
            <ChevronDown 
      ...
      {isExpanded && hasContent && (
        ...
        >
          {tomorrowItems.tasks.map((task) => (
            <div key={task.id} className="text-xs flex items-center gap-2" style={{ color: 'var(--text-secondary)' }}>
              <CheckCircle2 className="w-3.5 h-3.5" style={{ color: 'var(--text-muted)' }} strokeWidth={1.5} />
              <span>{task.title}</span>
            </div>
          ))}
          {tomorrowItems.meals.map((meal) => (
            <div key={meal.id} className="text-xs flex items-center gap-2" style={{ color: 'var(--text-secondary)' }}>
              <UtensilsCrossed className="w-3.5 h-3.5" style={{ color: 'var(--text-muted)' }} strokeWidth={1.5} />
              <span>{meal.name}</span>
            </div>
          ))}
        </div>
